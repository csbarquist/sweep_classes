...
class sweep:
    ...
    mems_freq_path = \
    '/home/vlu/Documents/Raw-Data/cd141121/MEMS'
    mems_ex_path = \
    ...
    tf_freq_path = \
    '/home/vlu/Documents/Research/4He/Data/cd141121/TF'
    tf_ex_path = \
    "/Users/cbarquist/Documents/Physics/Research/4He/Data/cd141121/Excitation/tf"
    ...
    def load_data(self, **kwargs):
        ...
        if kwargs['device'].lower() == 'mems':
            if kwargs['method'].lower() == 'f':
                self.f,self.x,self.y,self.r,self.temp = \
                ...
                self.ex,self.x,self.y,self.r,self.temp = \
        ...
        self.x = self.x * 1000000 * np.sqrt(2)
        self.y = self.y * 1000000 * np.sqrt(2)         
        self.r = self.r * 1000000 * np.sqrt(2) 
       
        #Calculate Temperature statistics
        self.avgTemp = np.average(self.temp)
        self.stdTemp = np.std(self.temp)
        self.maxTemp = np.amax(self.temp)
        self.minTemp = np.amin(self.temp)
...
#------------------------------------------------------------------------------
    ...
    def check_fit(self):
        ...
        if self.x_fit.any() and self.y_fit.any():
            ...
            plt.plot(self.f, self.x, 'k*', self.f, self.x_fit, 'g-')
...
#------------------------------------------------------------------------------
    ...
    def quadPlot(sweeps, **kwargs): #sweeps should be a list
       ...
       #If quick exists then the function will load sweeps and plot them.
        ...
        sup[0,0].set_ylabel("Output (mV)")
        
        sup[0,1].set_title("Y")
        sup[0,1].set_xlabel("Frequency (KHz)")
        sup[0,1].set_ylabel("Output (mV)")

        sup[1,0].set_title("R")
        sup[1,0].set_xlabel("Frequency (KHz)")
        sup[1,0].set_ylabel("Output (mV)")
        
        sup[1,1].set_title("X-Y")
        sup[1,1].set_xlabel("Output (mV)")
        sup[1,1].set_ylabel("Output (mV)")
...
#------------------------------------------------------------------------------
    ...
    def print_attributes(self):
        ...
        temp = 'Temp: ' + str(int(self.avgTemp*10)/10) +' +/- ' +\
            str(int(self.stdTemp*10)/10) + ' mK'
        amp = 'A = ' + str(self.A) + ' µVrms'
        f0 = 'f0 = ' + str(self.f0) + ' Hz'
        df = 'df = ' + str(self.df) + ' Hz'
        ph = 'ph = ' + str(self.ph) + ' deg'
        bg_0 = 'bg_0 = ' + str(self.bg_0) + ' µVrms'
        bg_1 = 'bg_1 = ' + str(self.bg_1) + ' µVrms/Vpp'
        print(idn + '\n' + temp + '\n' + amp + '\n' + f0 + '\n' + df + '\n' +
            ph + '\n' + bg_0 + '\n' + bg_1)
